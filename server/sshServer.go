...
func NewSSHServer(ts *TunnelServer) (*SSHServer, error) {
	...
	authorizedKeysBytes, err := os.ReadFile(ts.AuthKeysPath)
	if err != nil {
			...
			case newChannel, ok := <-chans:
				...
				if newChannel.ChannelType() != "tunnel" && newChannel.ChannelType() != "session" {
					s.tunnelServ.logger.Debug("Rejecting channel",
						zap.String("remote_addr", remoteAddr),
						zap.String("type", newChannel.ChannelType()))
					if err := newChannel.Reject(ssh.UnknownChannelType, "only tunnel and session channels are supported"); err != nil {
						s.tunnelServ.logger.Error("Failed to reject channel", zap.Error(err))
						continue
					}
				}
		...
		case "shell", "pty-req":
			...
			if req.WantReply {
				...
				if err := req.Reply(ok, nil); err != nil {
					s.tunnelServ.logger.Error("Failed to reply to request", zap.Error(err))
				}
			}
			if req.Type == "shell" {
				s.tunnelServ.logger.Info("Sending shell not supported message")
				if _, err := io.WriteString(channel, "Interactive shell is not supported. This is a tunnel server.\n"); err != nil {
					s.tunnelServ.logger.Error("Failed to write to channel", zap.Error(err))
				}
	...
	for req := range requests {
		...
		case "tunnel-request":
			...
			if tunnelReq.SubdomainSeed == "" {
				...
				if req.WantReply {
					if err := req.Reply(false, []byte(errMsg)); err != nil {
						s.tunnelServ.logger.Error("Failed to reply to request", zap.Error(err))
					}
				}
				continue
		...
		case "ping":
			...
			if tunnel == nil {
				...
				if req.WantReply {
					if err := req.Reply(false, []byte("tunnel not established")); err != nil {
						s.tunnelServ.logger.Error("Failed to reply to request", zap.Error(err))
					}
				}
				continue
			...
			if err != nil {
				s.tunnelServ.logger.Error("target port is not accessible",
					...
					zap.Error(err))
				if req.WantReply {
					if err := req.Reply(false, []byte(fmt.Sprintf("target port not accessible: %v", err))); err != nil {
						s.tunnelServ.logger.Error("Failed to reply to request", zap.Error(err))
					}
				}
				continue
...
// generateID generates a random ID for tunnels
func generateID() string {
	b := make([]byte, 16)
	if _, err := cryptorand.Read(b); err != nil {
		// En cas d'erreur, on retourne un ID basé sur le timestamp
