...
// waitForServer attempts to connect to the server until it succeeds or times out
...
func TestInteractiveSessionHandling(t *testing.T) {
	...
	runWithTimeout(t, "TestInteractiveSessionHandling", testTimeout, func(ctx context.Context) error {
		...
		defer func() {
			testLogger.Info("Closing SSH client")
			client.Close()
		}()
